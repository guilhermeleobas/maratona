  ...
  while (c--){
    ...
    for (int i=0; i<v.size(); i++){
      for (int j=i+1; j<v.size(); j++){
        double dist = dist_point (v[i], v[j]);
        pairs.push_back (make_pair (dist, make_pair(i, j)));
        pairs.push_back (make_pair (dist, make_pair(j, i)));
      }
    }

    sort (begin(pairs), end(pairs));

    cout << fixed << setprecision(2) << kruskal (pairs, n) << endl;
  }
