/*
Problema - Cebola
http://br.spoj.com/problems/CEBOLA/
Guilherme Leobas
*/
...
int convexHull (vector<point> &v, vector<bool> &used){

	int N = v.size();
	int start, p, q, camadas = 0;
	vector<bool> local = used;

	// Necessario no minimo três pontos;
	if (N < 3)
		return -1;

	// P = Ponto mais a esquerda;
	p = -1;
